...
parser = argparse.ArgumentParser(
    description="Download a file from S3 in parallel", prog="s3-mp-download")
parser.add_argument("src", help="The S3 key to download")
parser.add_argument("dest", help="The destination file")
parser.add_argument(
    "-np", "--num-processes", help="Number of processors to use",
    type=int, default=2)
parser.add_argument(
    "-s", "--split", help="Split size, in Mb", type=int, default=32)
parser.add_argument(
    "-f", "--force", help="Overwrite an existing file", action="store_true")
parser.add_argument(
    "--insecure", dest='secure', help="Use HTTP for connection",
    default=True, action="store_false")
parser.add_argument(
    "-t", "--max-tries", help="Max allowed retries for http timeout",
    type=int, default=5)
parser.add_argument(
    "-v", "--verbose", help="Be more verbose", default=False,
    action="store_true")
parser.add_argument(
    "-q", "--quiet", help="Be less verbose (for use in cron jobs)",
...
def do_part_download(args):
    ...
    """
    (
        bucket_name, key_name, fname, min_byte, max_byte, split, secure,
        max_tries, current_tries) = args
    conn = boto.connect_s3(calling_format=OrdinaryCallingFormat())
    conn.is_secure = secure

    # Make the S3 request
    resp = conn.make_request(
        "GET", bucket=bucket_name, key=key_name,
    ...
    logger.debug(
        "Reading HTTP stream in %dM chunks" % (chunk_size / 1024. / 1024))
    t1 = time.time()
    s = 0
    try:
        while True:
        ...
        s = s / 1024 / 1024.
        logger.debug(
            "Downloaded %0.2fM in %0.2fs at %0.2fMBps" % (s, t2, s / t2))
    except Exception, err:
        logger.debug(
            "Retry request %d of max %d times" % (current_tries, max_tries))
        if (current_tries > max_tries):
            ...
            current_tries += 1
            do_part_download(
                bucket_name, key_name, fname, min_byte, max_byte, split,
                secure, max_tries, current_tries)
...
def main(
        src, dest, num_processes=2, split=32, force=False, verbose=False,
        quiet=False, secure=True, max_tries=5):
    ...
    if size < 1024 * 1024:
        ...
        def arg_iterator(num_parts):
            for min_byte, max_byte in gen_byte_ranges(size, num_parts):
                yield (
                    bucket.name, key.name, dest, min_byte, max_byte, split,
                    secure, max_tries, 0)
        ...
        try:
            ...
            pool.map_async(
                do_part_download, arg_iterator(num_parts)).get(9999999)
            t2 = time.time() - t1
            logger.info(
                "Finished downloading %0.2fM in %0.2fs (%0.2fMBps)" %
                (s, t2, s / t2))
...
if __name__ == "__main__":
    ...
    arg_dict = vars(args)
    if arg_dict['quiet'] is True:
        logger.setLevel(logging.WARNING)
    if arg_dict['verbose'] is True:
