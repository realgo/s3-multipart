...
parser = argparse.ArgumentParser(
    description="Transfer large files to S3", prog="s3-mp-upload")
...
parser.add_argument(
    ...
    type=int, default=2)
parser.add_argument(
    "-f", "--force", help="Overwrite an existing S3 key", action="store_true")
parser.add_argument(
    "-s", "--split", help="Split size, in Mb", type=int, default=50)
parser.add_argument(
    "-rrs", "--reduced-redundancy",
    help="Use reduced redundancy storage. Default is standard.",
...
def do_part_upload(args):
    ...
    try:
        ...
        logger.info(
            "Uploaded part %s (%0.2fM) in %0.2fs at %0.2fMBps"
            % (i + 1, s, t2, s / t2))
        ...
        if (current_tries > max_tries):
            ...
            current_tries += 1
            do_part_download(
                bucket_name, mpu_id, fname, i, start, size, secure,
                max_tries, current_tries)


def main(
        src, dest, num_processes=2, split=50, force=False,
        reduced_redundancy=False, verbose=False, quiet=False, secure=True,
        max_tries=5):
    ...
    if size < 5 * 1024 * 1024:
        ...
        s = size / 1024. / 1024.
        logger.info(
            "Finished uploading %0.2fM in %0.2fs (%0.2fMBps)"
            % (s, t2, s / t2))
        return
    ...
    try:
        ...
        t1 = time.time()
        pool.map_async(
            do_part_upload, gen_args(num_parts, fold_last)).get(9999999)
