...
parser = argparse.ArgumentParser(
    description="Copy large files within S3", prog="s3-mp-copy")
parser.add_argument("src", help="The S3 source object")
parser.add_argument("dest", help="The S3 destination object")
parser.add_argument(
    "-np", "--num-processes", help="Number of processors to use", type=int,
    default=2)
parser.add_argument(
...
parser.add_argument(
    ...
    help="Use reduced redundancy storage. Default is standard.", default=False,
    action="store_true")
parser.add_argument(
...
def do_part_copy(args):
    ...
    (src_bucket_name, src_key_name, dest_bucket_name, mpu_id, part_num,
    ...
    src_bucket.get_key(src_key_name)

    # Do the copy
    t1 = time.time()
    mpu.copy_part_from_key(src_bucket_name, src_key_name, part_num,
                           start_pos, end_pos)
...
def main(src, dest, num_processes=2, split=50, force=False,
         reduced_redundancy=False, verbose=False):
